...
async function errorHandler(ctx, next) {
  try {
    ...
    if (err instanceof errors.ValidationError) {
      ctx.body = { status: 422, error: formatValidationError(err) }
      ctx.status = 422
    } else if (err instanceof errors.AppError) {
      ctx.body = { status: 422, error: formatException(err) }
      ctx.status = err.status
    } else if (err.status && err.status >= 400 && err.status < 500) {
      ctx.body = { status: 400 ,error: err.message }
      ctx.status = err.status
    }
...
function formatException(err) {
  ...
  if (idx !== -1) {
    ...
    return { [path]: message, status: status }
  }
  return message
}
